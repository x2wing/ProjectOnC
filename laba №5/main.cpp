...
void DeleteItems(string DataItem)
{
 ...
 {//пока не конец списка
    ...
    {
        if (elem==head)
            head=(*elem).pNext; // полукостыль, если первый элемент (всмысле последний) головной = удаляемому
        else
        prev->pNext = elem->pNext;//хвостик меняет жопу

        return;
    }

    prev = elem; //предыдущий стал 5-ый
    elem = elem->pNext; //текущий стал 4-ый elem поменялось ваш К.О.
  }
